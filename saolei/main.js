...
class Info extends React.Component{
    ...
    render(){
        ...
        return <div className='Info'>
            <div className='inp-group'><label>所有雷数(大于0，小于总格数)：</label><input name='minenum' type='text' value={minenum} onChange={this.handleMineNumInp} /></div>
            ...
            <div className='inp-group'><label>宽度(最大60)：</label><input name='width' type='text' value={width} onChange={this.handleWidthInp} /></div>
            <div className='inp-group'><label>长度(最大60)：</label><input name='height' type='text' value={height} onChange={this.handleHeightInp} /></div>
        </div>;
    }
}
    ...
    render(){
        ...
        var view = grid.map(function(row, xindex){
            return <div>{row.map(function(val, yindex){
                ...
                return <Cell pos={[xindex,yindex]} status={val} handleEvent={handleEvent}/>;
            })}</div>;
        });
